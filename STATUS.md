...
|---|---|---|---|
| Alan · Graph | kickoff | Restore PR #2 canonical schema to v3.1 table-per-type; tighten re-plan/idempotency/staleness guards; upsert TransferPoints idempotency claim; complete RCG-10 mutation adapter | nothing |
...
| Jun 21 | Polymorphic MVP schema kept experimental only | Alan | canonical `schema/schema.sql` remains v3.1 table-per-type; optional copy under `schema/experimental/polymorphic/` |
