...
> The core backend pieces are in place; spec 07 HTTP service is implemented in PR #29 (`raq/demo-mocks`). The remaining work is the _visible demo_ shell, benchmark/baselines, and the manual Clerk smoke on the live API. Protect those; add no new backend features.
...
**Net:** Val can start and largely finish the shell + sidebar on mocks in two days. What she cannot do alone is show _real_ data — that needs Raq's query→plan endpoint by Fri.
