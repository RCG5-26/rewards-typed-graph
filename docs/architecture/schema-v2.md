...
### 1.8a TransferBonus *(new in v2.0 — deferred write in MVP, required for Layer 4 demo)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
...
### 1.9 UserBalance *(new in v2.0 — CRITICAL for WalletAgent demo)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
| `user_id` | UUID FK → User | |
...
### 1.10 UserProgramStatus *(new in v2.0 — required for redemption agent reasoning)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
| `user_id` | UUID FK → User | |
...
### 1.11 MutationProposal *(new in v2.0 — required for Layer 4 verifier demo)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
...
### 1.13 PlanStep *(updated in v2.0 — HIGHEST RISK NODE)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
...
### 1.15 Evaluation *(updated in v2.0)*
| Property | Type | Notes |
|---|---|---|
| `id` | UUID | |
| `plan_id` | UUID FK → Plan | |
...
### 2.4 DEPENDS_ON — PlanStep → PlanStep *(intra-plan ordering and data dependencies)*
| Property | Type | Notes |
|---|---|---|
...
### 2.5 DEPENDS_ON_STATE — PlanStep → (any node in world or personal graph) *(new in v2.0 — CRITICAL)*
...
### 2.12 HOLDS_BALANCE — User → UserBalance *(new in v2.0)*
No edge properties. Navigational edge for the personal graph traversal pattern.

---

### 2.13 HAS_STATUS — User → UserProgramStatus *(new in v2.0)*
No edge properties. Navigational edge.

---

### 2.14 PROPOSED_BY — MutationProposal → AgentRun *(new in v2.0)*
...
*Document owner: Person A (Graph/Persistence). v2.0 prepared for Day 1 team discussion. Freeze after ratification.*
