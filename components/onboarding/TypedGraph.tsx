...
const LAYOUTS: Record<number, [number, number][]> = {
  2: [[-0.46, 0.14], [0.44, 0.54]],
  3: [[-0.52, 0.12], [0.10, 0.5], [0.52, 0.3]],
  4: [[-0.54, 0.11], [-0.14, 0.46], [0.3, 0.26], [0.56, 0.6]],
  5: [[-0.58, 0.1], [-0.26, 0.42], [0.06, 0.24], [0.36, 0.54], [0.6, 0.34]],
};
...
function buildChain(graph: PlanGraph): Hub[] {
  ...
  return ids.slice(0, n).map((id, i) => {
    ...
    return { id, label: node.label, stale: node.state === "stale", x: pos[i][0], depth: pos[i][1] };
  });
}
